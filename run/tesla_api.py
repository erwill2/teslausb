...
class TeslaAPI:
    def __init__(self):
        ...
        self._list_vehicles_cache = None
        self._list_vehicles_cache_time = 0
    ...
    def _load_tesla_api_json(self):
        ...
        try:
            ...
            loaded_json = json.loads(json_string, object_hook=datetime_parser)
            self.tesla_api_json.clear()
            self.tesla_api_json.update(loaded_json)
    ...
    def list_vehicles(self, force_refresh=False):
        # Cache the result briefly to avoid spamming the API in tight loops.
        if (self._list_vehicles_cache is None
                or force_refresh
                or (time.time() - self._list_vehicles_cache_time > 5)):
            self._list_vehicles_cache = self._execute_request(self.list_url, None, None, False)
            self._list_vehicles_cache_time = time.time()
        return self._list_vehicles_cache
...
_DEFAULT_API = TeslaAPI()

# Backwards-compatible module-level state for callers that import this script.
list_url = _DEFAULT_API.list_url
base_url = _DEFAULT_API.base_url
SETTINGS = _DEFAULT_API.SETTINGS
date_format = _DEFAULT_API.date_format
tesla_api_json = _DEFAULT_API.tesla_api_json
mutable_dir = _DEFAULT_API.mutable_dir


def _invalidate_access_token():
    return _DEFAULT_API._invalidate_access_token()


def _execute_request(url=None, method=None, data=None, require_vehicle_online=True):
    return _DEFAULT_API._execute_request(url, method, data, require_vehicle_online)


def _rest_request(url, method=None, data=None):
    return _DEFAULT_API._rest_request(url, method, data)


def _get_api_token():
    return _DEFAULT_API._get_api_token()


def _get_id():
    return _DEFAULT_API._get_id()


def _load_tesla_api_json():
    return _DEFAULT_API._load_tesla_api_json()


def _write_tesla_api_json():
    return _DEFAULT_API._write_tesla_api_json()


def _get_log_timestamp():
    return _DEFAULT_API._get_log_timestamp()


def _log(msg, flush=True):
    return _DEFAULT_API._log(msg, flush)


def _error(msg, flush=True):
    return _DEFAULT_API._error(msg, flush)


def list_vehicles(force_refresh=False):
    return _DEFAULT_API.list_vehicles(force_refresh)


def get_service_data():
    return _DEFAULT_API.get_service_data()


def get_vehicle_summary():
    return _DEFAULT_API.get_vehicle_summary()


def get_vehicle_legacy_data():
    return _DEFAULT_API.get_vehicle_legacy_data()


def get_nearby_charging():
    return _DEFAULT_API.get_nearby_charging()


def get_vehicle_data():
    return _DEFAULT_API.get_vehicle_data()


def get_vehicle_online_state():
    return _DEFAULT_API.get_vehicle_online_state()


def is_vehicle_online():
    return _DEFAULT_API.is_vehicle_online()


def get_charge_state():
    return _DEFAULT_API.get_charge_state()


def get_climate_state():
    return _DEFAULT_API.get_climate_state()


def get_drive_state():
    return _DEFAULT_API.get_drive_state()


def get_gui_settings():
    return _DEFAULT_API.get_gui_settings()


def get_vehicle_state():
    return _DEFAULT_API.get_vehicle_state()


def get_odometer():
    return _DEFAULT_API.get_odometer()


def is_car_locked():
    return _DEFAULT_API.is_car_locked()


def is_sentry_mode_enabled():
    return _DEFAULT_API.is_sentry_mode_enabled()


def streaming_ping():
    return _DEFAULT_API.streaming_ping()


def wake_up_vehicle():
    return _DEFAULT_API.wake_up_vehicle()


def set_charge_limit(percent):
    return _DEFAULT_API.set_charge_limit(percent)


def actuate_trunk():
    return _DEFAULT_API.actuate_trunk()


def actuate_frunk():
    return _DEFAULT_API.actuate_frunk()


def flash_lights():
    return _DEFAULT_API.flash_lights()


def set_sentry_mode(enabled: bool):
    return _DEFAULT_API.set_sentry_mode(enabled)


def enable_sentry_mode():
    return _DEFAULT_API.enable_sentry_mode()


def disable_sentry_mode():
    return _DEFAULT_API.disable_sentry_mode()


def toggle_sentry_mode():
    return _DEFAULT_API.toggle_sentry_mode()
...
def _setup_settings(args, api=None):
    """
    Populates the API settings from command-line arguments and environment
    variables. Also handles API token setup.
    """
    if api is None:
        api = _DEFAULT_API
...
def _parse_kwargs(arguments_str):
    """
    Parses comma-separated key:value arguments into a dictionary and returns
    both the dictionary and its string representation.
    """
    kwargs = {}
    if arguments_str:
...
def _print_result(result, api=None):
    """
